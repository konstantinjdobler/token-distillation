# copied from https://github.com/bminixhofer/zett/blob/016775824664d3479dfd6215d78fd73d51dce833/zett/utils.py
# adaptations (apart from `ruff` formatting) marked with [CHANGE]
...
from transformers import AutoTokenizer, ByT5Tokenizer, PreTrainedTokenizerFast
...
# [CHANGE] comment out because we don't want to include the data file, it's not used in our code anyways
...
def make_whitespace_consistent(tokenizer, maxlen):
    ...
    consistent_tokenizer._tokenizer.model = models.Unigram(pieces, unk_id=tokenizer.unk_token_id)
...
def copy_tokenizer_auxiliaries(source, target):
    ...
    if source.get_vocab() == target.get_vocab() and has_same_special_tokens(source, target):
        return target

    needs_piece_update = False
    for source_token in source.all_special_tokens:
        if source_token not in target.all_special_tokens or source.convert_tokens_to_ids(
            source_token
    ...
    if needs_piece_update:  # only implemented for Unigram
        ...
        pieces = [piece for piece in pieces if piece[0] not in source.all_special_tokens]

        for i in np.argsort(source.all_special_ids):
            pieces.insert(source.all_special_ids[i], (source.all_special_tokens[i], 0.0))
...
def get_sample_indices(n, p, batch_size, min_k, n_samples):
    ...
    for i in range(n_samples):
        ...
        indices[i, :num_random] = random_indices[random_offset : random_offset + num_random]
        ...
        indices[i, num_random:] = np.random.choice(n, size=batch_size - num_random, p=sample_p, replace=False)
...
def load_params(model_name_or_path, **kwargs):
    try:
        index = cached_file(model_name_or_path, "flax_model.msgpack.index.json", **kwargs)
    except OSError:
        index = None

    if index is not None:
        index = json.load(open(index))
        files = [cached_file(model_name_or_path, x, **kwargs) for x in set(index["weight_map"].values())]
    else:
